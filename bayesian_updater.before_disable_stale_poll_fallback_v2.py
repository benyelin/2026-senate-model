...
def run_bayesian_update(
    ...
    # Remove prior Bayesian outputs and transient calculation fields so
    # rerunning this updater cannot create _x/_y duplicate columns.
    stale_bayes_columns = BAYES_COLUMNS + [
        "prior_margin_dem",
        "polling_margin_used",
    ...
    races = races.drop(
        columns=[c for c in stale_bayes_columns if c in races.columns],
        errors="ignore",
    )
    ...
    merged = races.merge(
        polling[["state", "polling_margin_dem", "poll_count", "effective_poll_count", "avg_poll_age_days", "total_poll_weight"]],
        on="state",
        how="left",
