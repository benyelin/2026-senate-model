...
def main():
    ...
    # Drop stale accelerator columns from prior runs before any merge.
    # This makes the script safe to run repeatedly in the pipeline.
    stale_premerge_cols = [
    ...
    if stale_premerge_cols:
        bayes = bayes.drop(columns=stale_premerge_cols, errors="ignore")
    ...
    # Also sync accelerated Bayesian values into race_inputs.csv if present.
    # This is a defensive step so the forecast cannot accidentally use stale pre-accelerator fields.
    ...
    if race_inputs_path.exists():
        ...
        if "state" in races.columns:
            ...
            sync = merged[["state_norm"] + sync_cols].drop_duplicates("state_norm").copy()

            races = races.merge(sync, on="state_norm", how="left", suffixes=("", "_accelerated"))

            # Standard model fields.
            if "bayesian_model_margin_dem_capped_accelerated" in races.columns:
                races["bayesian_model_margin_dem"] = races[
                    "bayesian_model_margin_dem_capped_accelerated"
                ].combine_first(races.get("bayesian_model_margin_dem"))

            if "bayesian_polling_weight_capped_accelerated" in races.columns:
                races["bayesian_polling_weight"] = races[
                    "bayesian_polling_weight_capped_accelerated"
                ].combine_first(races.get("bayesian_polling_weight"))

            if "bayesian_fundamentals_weight_capped_accelerated" in races.columns:
                races["bayesian_fundamentals_weight_capped"] = races[
                    "bayesian_fundamentals_weight_capped_accelerated"
                ].combine_first(races.get("bayesian_fundamentals_weight_capped"))

            # Keep audit fields on race_inputs too.
            for c in [
                "polling_confidence_boost",
                "polling_confidence_absolute_cap",
                "polling_confidence_weight_change",
                ...
                acc = f"{c}_accelerated"
                if acc in races.columns:
                    races[c] = races[acc].combine_first(races[c] if c in races.columns else pd.Series(index=races.index, dtype=float))
