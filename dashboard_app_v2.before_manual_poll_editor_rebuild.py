...
with tab_manual_polls:
    ...
    st.caption("Editor schema includes: poll_sponsor_type, partisan_sponsor_party, is_internal_poll, pollster_partisan_affiliation.")
    st.caption("Manual poll editor schema: NO manual house-effect field; poll adjustments are generated by pipeline scripts.")
    ...
    # Force structured poll metadata columns to UI-compatible types.
    text_metadata_cols = [
        "poll_sponsor_type",
        "partisan_sponsor_party",
        "pollster_partisan_affiliation",
    ...
    if "is_internal_poll" in existing_manual_polls.columns:
    ...
    st.markdown("### Partisan Poll Metadata")

    partisan_metadata_cols = [
        "race",
        "state",
        "pollster",
        "poll_sponsor_type",
    ...
    partisan_metadata_view = editable[
        [c for c in partisan_metadata_cols if c in editable.columns]
    ].copy()

    st.data_editor(
        partisan_metadata_view,
        use_container_width=True,
        hide_index=True,
        num_rows="fixed",
        key="partisan_metadata_editor_v1",
        column_config={
    ...
    st.markdown("### Full Manual Poll Editor")
    ...
    edited = st.data_editor(
        ...
        key="manual_poll_editor_v4_partisan_fields_visible",
        column_config={
            "poll_sponsor_type": st.column_config.SelectboxColumn(
                "Sponsor type",
                options=["", "independent", "media", "university", "party", "campaign", "super PAC", "other"],
