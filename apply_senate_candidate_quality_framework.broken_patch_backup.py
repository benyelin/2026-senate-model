...
def main():
    ...
    parser.add_argument(
        "--keep-old-ad-hoc",
        action="store_true",
        help="When applying, keep old ad hoc candidate adjustment columns. Default is to zero them so the framework replaces the full old candidate-side system.",
    )
    ...
    # Preserve any manually entered scandal adjustment/rationale already in race_inputs.csv
    # if it exists. This lets manual scandal inputs live in race_inputs.csv as the source of truth.
    for col, default in [
        ("candidate_scandal_adjustment_dem", 0.0),
        ("candidate_scandal_rationale", ""),
    ]:
        ensure_col(races, col, default)
    ...
    # Merge manual scandal fields from race_inputs onto audit-derived framework.
    ...
    # Recompute final framework candidate-quality adjustment using manual scandal field
    # as the only manual exception.
    ...
    audit_for_merge["candidate_quality_framework_version"] = "senate_candidate_quality_framework_v2"
    ...
    if current_col is None:
        ...
        current_col = "candidate_quality_adjustment_dem"

    races[current_col] = safe_num(races[current_col], 0.0)
    ...
    review["current_candidate_quality_adjustment_dem"] = safe_num(review[current_col], 0.0)

    # Calculate the full current candidate-side adjustment using legacy components.
    # This is the number the framework is intended to replace once applied.
    review["current_full_candidate_side_adjustment_dem"] = review["current_candidate_quality_adjustment_dem"]

    for legacy_col in OLD_AD_HOC_COLS:
        if legacy_col in review.columns:
            review["current_full_candidate_side_adjustment_dem"] += safe_num(review[legacy_col], 0.0)
    ...
    # Narrow change: candidate_quality_adjustment_dem column only.
    review["candidate_quality_change_dem"] = (
        review["proposed_candidate_quality_adjustment_dem"]
        - review["current_candidate_quality_adjustment_dem"]
    )

    # Full-system change: proposed framework minus current quality + old ad hoc components.
    ...
    print()
    print("Largest proposed candidate-quality changes:")
    ...
    if not args.apply:
        ...
        print("Default apply replaces full old candidate-side system by zeroing legacy ad hoc columns.")\n        print("To apply but keep old ad hoc columns: python3 apply_senate_candidate_quality_framework.py --apply --keep-old-ad-hoc")
        return
