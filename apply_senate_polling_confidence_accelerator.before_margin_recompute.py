...
def identify_weight_columns(df):
    candidates = []
    ...
    for col in df.columns:
        ...
        if "poll" in low and "weight" in low:
            ...
            candidates.append(col)
        elif "poll" in low and "cap" in low:
            candidates.append(col)

    # Prefer final/capped applied fields if present.
    priority_terms = [
        "capped",
        "final",
        "applied",
    ...
    def score(col):
        low = col.lower()
        return sum(term in low for term in priority_terms)

    candidates = sorted(set(candidates), key=score, reverse=True)

    if not candidates:
        raise ValueError(
            "Could not identify a polling/Bayesian weight column to adjust. "
            "Inspect inputs/bayesian_update_generated.csv columns."
        )
...
def main():
    ...
    weight_cols = identify_weight_columns(bayes)

    # Choose the most likely model-driving weight column as the first candidate.
    ...
    print()
    print(f"Adjusted Bayesian polling weight column: {weight_col}")
