...
# For now, officeholding and special circumstances are audit placeholders.
# Later we can make these explicit columns in race_inputs.csv.
DEFAULT_OFFICEHOLDING_ADJUSTMENT_DEM = 0.0
DEFAULT_SPECIAL_CANDIDATE_ADJUSTMENT_DEM = 0.0
...
def main():
    ...
    out["candidate_officeholding_adjustment_dem"] = DEFAULT_OFFICEHOLDING_ADJUSTMENT_DEM
    out["candidate_special_adjustment_dem"] = DEFAULT_SPECIAL_CANDIDATE_ADJUSTMENT_DEM

    out["framework_candidate_quality_adjustment_dem"] = (
        out["candidate_war_adjustment_dem"]
        + out["candidate_officeholding_adjustment_dem"]
        + out["candidate_special_adjustment_dem"]
    ).clip(lower=-FRAMEWORK_CAP, upper=FRAMEWORK_CAP)
    ...
    out["framework_candidate_quality_method"] = (
        "Audit-only v1: 50% of Senate WAR-audit proposed adjustment, capped at +/-1.5; "
        "officeholding and special-circumstance adjustments set to 0 pending later framework steps."
    )
    ...
    output_cols = [
        ...
        "candidate_officeholding_adjustment_dem",
        "candidate_special_adjustment_dem",
